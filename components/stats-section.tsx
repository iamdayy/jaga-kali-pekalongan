...
import { Droplets, AlertCircle, CheckCircle2, Users } from "lucide-react"
...
async function fetchStats() {
  try {
    ...
    const confirmations = reports.reduce((sum: number, r: any) => sum + (r.confirmations_count || 0), 0)
    ...
    <section id="statistik" className="py-12 md:py-24 bg-background">
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold text-foreground mb-4">Statistik Real-Time</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {statItems.map((item) => {
            ...
            return (
              <div key={item.label} className={`rounded-lg border border-border p-6 ${item.bgColor}`}>
